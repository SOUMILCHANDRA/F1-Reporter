...
class _RaceHubScreenState extends ConsumerState<RaceHubScreen> {
  ...
  final List<String> sessions = ['Race', 'Qualifying', 'Sprint', 'FP1', 'FP2', 'FP3'];
  ...
  Widget _buildEventPicker(AsyncValue<List<dynamic>> schedule) {
    return schedule.when(
      data: (events) {
        final currentEvent = events.firstWhere((e) => e['round_number'] == selectedRound, orElse: () => events[0]);
        return PopupMenuButton<int>(
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.location_on, color: AppConfig.accentRed, size: 18),
              const SizedBox(width: 8),
              Text(
                'RD $selectedRound / ${currentEvent['event_name']}'.toUpperCase(),
  ...
  Widget _buildSessionPicker() {
    return Container(
      height: 40,
      decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: AppConfig.border))),
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                border: Border(bottom: BorderSide(color: active ? AppConfig.accentRed : Colors.transparent, width: 2)),
              ),
              child: Text(
                t.toUpperCase(),
                style: TextStyle(
  ...
  Widget _buildActiveTabContent() {
    ...
    switch (activeSubTab) {
      case 'Results': return ResultsTab(year: selectedYear, round: selectedRound, session: selectedSession);
      case 'Laps': return LapsTab(year: selectedYear, round: selectedRound, session: selectedSession);
      case 'Tyres': return TyresTab(year: selectedYear, round: selectedRound, session: selectedSession);
